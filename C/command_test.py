...
from tkinter import messagebox, Menu, Listbox, Scrollbar, PhotoImage # type: ignore
...
'''
    ...
    def btn_c_(self):
        command = API.get_command_API()(root, self.roots)
        ...
        command.print('afhgd')
        command.input(lambda text: command.print(text), 'hgfks')
        command.print('3ehjg')
        command.input(lambda text: command.print(text), 'hiken')
        command.print('8v6fr')
        command.print('cwede')
        command.print('ajkce')
        command.input(lambda text: command.print(text), 'aidee')
    ...
    def show_main_ui(self):
        ...
        if language == 'zh-cn':
            ...
            command_ = tk.Button(btn_f, text='a', command=self.btn_c_)
            command_.pack(side=tk.LEFT, padx=0, pady=0)
    ...
    def system_(self):
        ...
        if language == 'zh-cn':
            ...
            temp_ = self.Version_()
            if '(BETA VERSION IN DEVELOPMENT)' in temp_:
                version_text: str = temp_.replace(' (BETA VERSION IN DEVELOPMENT)', '（开发中的测试版本)')
            else:
                version_text = temp_

            about_sys = tk.Label(sys_root, text=f"系统版本：{version_text}")
            about_sys.pack()
